    ...
    <DashboardLayout>
      <div className="space-y-8 pb-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-[10px] font-black tracking-[0.3em] text-taysir-teal/40 uppercase">Dossier Académique</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          <div className="col-span-1 md:col-span-8 bento-card p-8 flex flex-col md:flex-row gap-8 items-center md:items-start bg-gradient-to-br from-white to-taysir-bg/30 relative overflow-hidden">
            ...
            <div className="flex-1 text-center md:text-left relative z-10">
              <div className="flex flex-col md:flex-row md:items-center gap-3 mb-4">
                ...
                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest self-center md:self-auto border ${
                  student.isActive 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 mt-6">
                <div className="flex items-center gap-3 text-sm font-bold text-taysir-teal/70">
                   <div className="p-2 bg-white rounded-lg shadow-sm"><Mail size={16} className="text-taysir-teal/30" /></div>
                   <span>{student.email || 'Non renseigné'}</span>
                </div>
                <div className="flex items-center gap-3 text-sm font-bold text-taysir-teal/70">
                   <div className="p-2 bg-white rounded-lg shadow-sm"><Phone size={16} className="text-taysir-teal/30" /></div>
                   <span>{student.isMinor ? student.parentPhone : student.phone || 'N/A'}</span>
                </div>
                <div className="flex items-center gap-3 text-sm font-bold text-taysir-teal/70">
                   <div className="p-2 bg-white rounded-lg shadow-sm"><Calendar size={16} className="text-taysir-teal/30" /></div>
                   <span>Inscrit le {new Date(student.registrationDate).toLocaleDateString()}</span>
                </div>
                <div className="flex items-center gap-3 text-sm font-bold text-taysir-teal/70">
                   <div className="p-2 bg-white rounded-lg shadow-sm"><MapPin size={16} className="text-taysir-teal/30" /></div>
                   <span className="truncate max-w-[200px]">{student.address || 'Adresse non saisie'}</span>
                </div>
              </div>

              {student.isMinor && (
                <div className="mt-8 p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-center gap-4">
                   <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-amber-600 shadow-sm font-black">P</div>
                   <div>
                      <p className="text-[10px] font-black text-amber-600/60 uppercase tracking-widest leading-none mb-1">Responsable Légal</p>
                      <p className="text-sm font-black text-amber-700 uppercase tracking-tight">{student.parentName}</p>
                   </div>
                </div>
              )}
            </div>
          ...
          <div className="col-span-1 md:col-span-4 grid grid-cols-1 gap-4">
             <div className="bento-card p-8 bg-taysir-teal text-white border-none relative overflow-hidden group">
                <div className="relative z-10 flex flex-col justify-between h-full">
                   <div className="flex justify-between items-start">
                      <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-sm"><Clock size={24} className="text-taysir-light" /></div>
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">Sessions</span>
                   </div>
                   <div className="mt-6">
                      <div className="text-5xl font-black tracking-tighter leading-none">{student.attendance.length}</div>
                      <div className="text-[10px] font-black uppercase tracking-widest opacity-60 mt-2">Présences enregistrées</div>
                   </div>
                </div>
             ...
             <div className="bento-card p-8 bg-white border border-taysir-teal/5 relative overflow-hidden group">
                <div className="relative z-10 flex flex-col justify-between h-full">
                   <div className="flex justify-between items-start">
                      <div className="p-3 bg-rose-50 rounded-2xl text-rose-500"><Wallet size={24} /></div>
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-taysir-teal/30">Trésorerie</span>
                   </div>
                   <div className="mt-6">
                      <div className="text-4xl font-black text-taysir-teal tracking-tighter leading-none">
                        {student.paymentPlans.reduce((acc: number, p: any) => acc + (p.totalAmount - p.paidAmount), 0).toLocaleString()} <span className="text-sm">DZD</span>
                      </div>
                      <div className="text-[10px] font-black uppercase tracking-widest text-rose-500 mt-2">Solde débiteur actuel</div>
                   </div>
                </div>
             </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="col-span-1 md:col-span-5 space-y-6">
            <div className="flex items-center gap-3">
               <div className="p-2 bg-taysir-teal/5 rounded-xl text-taysir-teal"><Users size={20} /></div>
               <h3 className="text-lg font-black text-taysir-teal uppercase tracking-tighter">Inscriptions Groupes</h3>
            </div>
            <div className="grid grid-cols-1 gap-3">
              {student.groups.length > 0 ? (
                student.groups.map((group: any) => (
                  <div key={group.id} className="p-5 bg-white rounded-[24px] border border-taysir-teal/5 flex items-center justify-between shadow-sm hover:shadow-md transition-all group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-sm font-black text-taysir-teal uppercase tracking-tight">{group.name}</div>
                        <div className="text-[9px] font-bold text-taysir-teal/30 uppercase tracking-widest">Actif depuis le {new Date(group.createdAt).toLocaleDateString()}</div>
                      </div>
                    </div>
                  </div>
                ))
                ...
                <div className="py-12 text-center bg-white rounded-[32px] border-2 border-dashed border-taysir-teal/5 opacity-40">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest">Aucun groupe affecté</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="col-span-1 md:col-span-7 space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                 <div className="p-2 bg-taysir-teal/5 rounded-xl text-taysir-teal"><FileText size={20} /></div>
                 <h3 className="text-lg font-black text-taysir-teal uppercase tracking-tighter">Dossier Numérique</h3>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {student.documents.length > 0 ? (
                student.documents.map((doc: any) => (
                  <div key={doc.id} className="p-5 bg-white rounded-[24px] border border-taysir-teal/5 flex items-center justify-between shadow-sm hover:border-taysir-accent/30 transition-all">
                    <div className="flex items-center gap-4">
                      <div className="p-3 bg-taysir-teal/5 rounded-2xl text-taysir-teal shadow-inner">
                        <FileText size={20} />
                      </div>
                      <div>
                        <div className="text-[11px] font-black text-taysir-teal uppercase tracking-tight truncate max-w-[120px]">{doc.name}</div>
                        <div className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded-md mt-1 w-fit ${
                          doc.status === 'APPROVED' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                        }`}>{doc.status}</div>
                      </div>
                    </div>
                    <a href={doc.url} target="_blank" className="p-2 hover:bg-taysir-teal hover:text-white rounded-xl transition-all text-taysir-teal">
                       <Eye size={18} />
                    </a>
                  </div>
                ))
                ...
                <div className="col-span-2 py-16 text-center bg-white rounded-[32px] border-2 border-dashed border-taysir-teal/5 opacity-40">
                  <FileText size={48} className="mx-auto mb-3 text-taysir-teal/20" />
                  <p className="text-xs font-black uppercase tracking-widest text-taysir-teal">Aucune pièce jointe</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-3">
             <div className="p-2 bg-taysir-teal/5 rounded-xl text-taysir-teal"><User size={20} /></div>
             <h3 className="text-lg font-black text-taysir-teal uppercase tracking-tighter">Journal des Présences</h3>
          </div>
          <div className="bento-card p-0 overflow-hidden bg-white border border-taysir-teal/5 shadow-xl">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-taysir-teal/5">
                  {student.attendance.map((record: any) => (
                    <tr key={record.id} className="hover:bg-taysir-teal/[0.01] transition-colors group">
                      <td className="px-8 py-6">
                        <div className="text-sm font-black text-taysir-teal uppercase tracking-tight">{new Date(record.session.startTime).toLocaleDateString()}</div>
                        <div className="text-[10px] font-bold text-taysir-teal/30 uppercase tracking-widest mt-1 flex items-center gap-1">
                          <Clock size={10} /> {new Date(record.session.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </div>
                      </td>
                      <td className="px-8 py-6">
            ...
            {student.attendance.length === 0 && (
              <div className="py-20 text-center opacity-30">
                <Calendar size={64} className="mx-auto mb-4 text-taysir-teal/20" />
                <p className="font-black uppercase tracking-[0.3em] text-xs">Historique vierge</p>
              </div>
            )}
          </div>
        </div>
