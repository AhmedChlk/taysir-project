...
import { X, Wallet, Calendar, AlertCircle, Users, PlusCircle, Clock, MapPin, User, ChevronRight } from 'lucide-react';
...
import { formatFullName, cn } from '@/utils/format';
...
export default function Drawer({ type, onClose, formData }: DrawerProps) {
  ...
  const getTitle = () => {
    switch (type) {
      case 'payments': return t('drawer_payments');
      case 'new-finance': return t('drawer_new_finance');
      case 'edit-staff': return t('drawer_edit_staff');
      case 'sessions': return t('drawer_sessions');
      case 'new-session': return t('drawer_new_session');
      default: return t('drawer_operational');
    }
  };

  const getIcon = () => {
    switch (type) {
      case 'payments': return <Wallet size={20} />;
      case 'new-finance': return <Wallet size={20} />;
      case 'edit-staff': return <Users size={20} />;
      case 'sessions': return <Calendar size={20} />;
      case 'new-session': return <PlusCircle size={20} />;
          ...
          <div className="space-y-4">
            {formData?.todaySessions?.length > 0 ? (
              formData.todaySessions.map((session: any) => (
                <div key={session.id} className="p-5 rounded-[24px] bg-white border border-taysir-teal/5 shadow-sm hover:shadow-md transition-all group">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-taysir-teal/5">
                    ...
                    <div className="flex items-center gap-2 text-xs font-bold text-taysir-teal">
                      ...
                      {formatFullName(session.instructor.firstName, session.instructor.lastName)}
                    </div>
                  </div>
                </div>
              ))
              ...
              <div className="py-20 text-center opacity-30">
                ...
                <p className="font-bold uppercase tracking-widest">{t("no_sessions_today")}</p>
              </div>
            )}
          </div>
        );
          ...
          <div className="space-y-4">
            {formData?.pendingPayments?.length > 0 ? (
              formData.pendingPayments.map((plan: any) => (
                <div key={plan.id} className="p-5 rounded-[24px] bg-white border border-taysir-teal/5 shadow-sm hover:shadow-md transition-all">
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      <h4 className="font-black text-taysir-teal uppercase tracking-tighter">
                        {formatFullName(plan.student.firstName, plan.student.lastName)}
                      </h4>
                    ...
                    <div className="text-right">
                      <div className="text-rose-500 font-black text-sm">
                        -{(plan.totalAmount - plan.paidAmount).toLocaleString()} DZD
                      </div>
                      <p className="text-[10px] font-bold text-rose-500/50 uppercase">{t("remaining")}</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {plan.tranches.slice(0, 2).map((tranche: any, idx: number) => (
                      <div key={tranche.id} className="flex items-center justify-between p-2 bg-taysir-bg rounded-xl text-[10px] font-bold uppercase tracking-tight">
                        <span className="opacity-40">Échéance {new Date(tranche.dueDate).toLocaleDateString()}</span>
              ...
              <div className="py-20 text-center opacity-30">
                ...
                <p className="font-bold uppercase tracking-widest">{t("all_payments_up_to_date")}</p>
              </div>
            )}
          </div>
        );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-taysir-teal/40 uppercase tracking-[0.2em] mt-1">
                {type.includes('new') ? 'Nouveau Dossier' : `Vue Opérationnelle`}
              </p>
            </div>
          </div>
          <button 
