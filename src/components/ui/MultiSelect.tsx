      ...
      <div className="relative">
        <div
          ...
          className={clsx(
            ...
            isOpen ? "border-taysir-teal ring-4 ring-taysir-teal/10" : "border-taysir-teal/15 hover:border-taysir-teal/30"
          )}
        >
          {value.length > 0 ? (
          ...
          <ChevronDown
            size={18}
            className={clsx("ms-auto text-gray-400 transition-transform", isOpen && "rotate-180")}
          />
        </div>
        ...
        {isOpen && (
          <div className="absolute z-50 mt-2 max-h-60 w-full overflow-y-auto rounded-2xl bg-white p-2 shadow-2xl ring-1 ring-black/5 custom-scrollbar">
            {options.map((option) => (
              ...
              >
                <span className="uppercase tracking-widest text-[10px]">{option.label}</span>
