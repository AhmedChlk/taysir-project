...
export default function PaymentPlanForm({ onSuccess, students = [], activities = [] }: PaymentPlanFormProps) {
  ...
  const [tranches, setTranches] = useState<{ amount: number; dueDate: string }[]>([
    { amount: 0, dueDate: new Date().toISOString().split('T')[0] }
  ]);

  const addTranche = () => {
    const lastDate = new Date(tranches[tranches.length - 1].dueDate);
    const nextDate = new Date(lastDate.setMonth(lastDate.getMonth() + 1));
    setTranches([...tranches, { amount: 0, dueDate: nextDate.toISOString().split('T')[0] }]);
  };
  ...
  const updateTranche = (index: number, field: 'amount' | 'dueDate', value: any) => {
  ...
  const distributeAmount = () => {
    ...
    const newTranches = tranches.map((t, i) => ({
      ...t,
      amount: i === tranches.length - 1 ? amountPerTranche + remainder : amountPerTranche
    }));
    setTranches(newTranches);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const calculatedTotal = tranches.reduce((sum, t) => sum + Number(t.amount), 0);
    if (calculatedTotal !== totalAmount) {
      setError(`La somme des tranches (${calculatedTotal} DZD) ne correspond pas au montant total (${totalAmount} DZD).`);
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center px-1">
          <h3 className="text-sm font-black text-taysir-teal uppercase tracking-widest">Échéancier (Tranches)</h3>
          <button
            type="button"
            onClick={addTranche}
        ...
        <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
          {tranches.map((tranche, index) => (
            <div key={index} className="flex items-center gap-3 p-4 bg-white border border-taysir-teal/10 rounded-2xl shadow-sm">
              ...
              <div className="flex-1">
                <Input
                  ...
                  onChange={(e) => updateTranche(index, 'dueDate', e.target.value)}
                  required
                />
              </div>
              <button
