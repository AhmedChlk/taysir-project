...
export default function SessionForm({ onSuccess, rooms = [], activities = [], staff = [], groups = [] }: SessionFormProps) {
  ...
  const instructors = staff.filter((u: any) => u.role === 'INTERVENANT' || u.role === 'GERANT' || u.role === 'ADMIN');
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Select
            ...
            options={[
              ...
              ...rooms.map(r => ({ label: `${r.name} (Cap. ${r.capacity})`, value: r.id }))
            ]}
            required
          />
          <Select
            ...
            options={[
              ...
              ...instructors.map(i => ({ label: formatFullName(i.firstName, i.lastName), value: i.id }))
            ]}
            required
          />
        </div>
