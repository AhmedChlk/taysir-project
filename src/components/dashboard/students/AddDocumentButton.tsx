...
export default function AddDocumentButton({ studentId }: AddDocumentButtonProps) {
  ...
  const handleUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (!uploadRes.success || !uploadRes.data?.url) {
        setErrorMessage(uploadRes.error || "Erreur lors de l'upload du fichier");
        return;
      }
  ...
  return (
    <>
      ...
      >
        <Plus size={18} />
        <span className="font-black text-[10px] uppercase tracking-widest">Ajouter un document</span>
      </button>
        ...
        footer={
          <>
            ...
            >
              ...
              <span className="font-black text-xs uppercase tracking-widest">Téléverser</span>
            </button>
          </>
        }
      >
        <form id="add-document-form" onSubmit={handleUpload} className="space-y-6 p-2">
          {errorMessage && (
          ...
          >
             ...
             <div className="text-center">
                <p className="text-sm font-black text-taysir-teal uppercase tracking-tight">Cliquer pour sélectionner</p>
                <p className="text-[10px] font-bold text-taysir-teal/40 uppercase tracking-widest mt-1">PDF, JPG, PNG (max 5Mo)</p>
             </div>
          </div>
