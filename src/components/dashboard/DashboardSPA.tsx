...
import { GraduationCap, LayoutDashboard, Search, Settings as SettingsIcon } from 'lucide-react';
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
        ...
        >
          ...
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-[10px] font-black tracking-[0.3em] text-taysir-teal/40 uppercase">{t("command_center")}</span>
            ...
            <h1 className="text-3xl md:text-4xl font-black tracking-tight text-taysir-teal uppercase">
              {t("dashboard")}<span className="text-taysir-accent">.</span>
            </h1>
          </div>
        </motion.div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-black text-taysir-teal/30 uppercase tracking-[0.2em]">{t("academic_calendar")}</span>
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="col-span-1 md:col-span-8 bento-card p-8 bg-gradient-to-br from-taysir-teal to-taysir-teal/90 text-white flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden shadow-2xl">
            <div className="relative z-10">
              <h3 className="text-2xl font-black uppercase tracking-tighter mb-2 italic">{t("need_help")}</h3>
              <p className="text-sm font-medium text-white/70 max-w-md leading-relaxed">
                {t("support_desc")}
              </p>
              <div className="flex gap-4 mt-8">
                <button className="bg-white text-taysir-teal px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all">
                  {t("user_guide")}
                </button>
                <button className="bg-taysir-accent text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:shadow-lg transition-all">
                  {t("contact_support")}
                </button>
              </div>
            </div>
            <div className="relative z-10 w-24 h-24 bg-white/10 rounded-[32px] backdrop-blur-md flex items-center justify-center border border-white/10 shadow-inner">
               <GraduationCap size={48} className="text-taysir-accent" strokeWidth={1} />
            </div>
