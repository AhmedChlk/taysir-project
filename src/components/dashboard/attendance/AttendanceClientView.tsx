...
import { Session, Student, Group, AttendanceRecord } from "@/types/schema";
import { StatutPresence } from "@prisma/client";
import { Check, X, Clock, Save, ChevronDown } from "lucide-react";
import { clsx } from "clsx";
import { useTranslations, useLocale } from "next-intl";
import { formatDate, formatTime, formatFullName } from "@/utils/format";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <label className="block text-sm font-medium text-gray-700 mb-2">{t("select_session")}</label>
          ...
          >
            {(sessions || []).map(session => (
              <option key={session.id} value={session.id}>
                {formatDate(session.startTime, locale)} - {formatTime(session.startTime, locale)} ({t("groups")}: {groupsMap[session.groupId || ""] || t("unknown")})
              </option>
            ))}
          </select>
              ...
              return (
                <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-medium text-gray-900">{formatFullName(student.firstName, student.lastName)}</div>
                    <div className="text-xs text-gray-500">{student.email}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        onClick={() => toggleStatus(student.id, StatutPresence.PRESENT)}
                        className={clsx(
                      ...
                      <button
                        onClick={() => toggleStatus(student.id, StatutPresence.RETARD)}
                        className={clsx(
                          "flex h-10 w-10 items-center justify-center rounded-lg border transition-all",
                      ...
                      <button
                        onClick={() => toggleStatus(student.id, StatutPresence.ABSENT)}
                        className={clsx(
                          "flex h-10 w-10 items-center justify-center rounded-lg border transition-all",
