...
export default function DashboardClientView({
	...
	const todaySessions = sessions.filter((s) =>
		isSameDay(new Date(s.startTime), today),
	);
			...
			<header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
				...
				>
					<div className="flex items-center gap-2 mb-1">
						...
						<span className="text-xs font-bold tracking-[0.2em] text-taysir-teal/60 uppercase">
							Espace Direction
						</span>
					</div>
			...
			<main className="grid grid-cols-1 md:grid-cols-12 gap-5 max-w-7xl mx-auto">
					...
					<div className="relative z-10 h-full flex flex-col justify-between">
						<div>
							...
							<h2 className="text-7xl md:text-8xl font-black tracking-tighter">
								{totalStudents}
								<span className="text-white/50 text-3xl ml-2 italic">
									élèves
								</span>
							</h2>
						...
						<div className="mt-12 flex gap-3">
							...
							<button
								type="button"
								className="btn-ghost text-white hover:bg-white/10 border border-white/20 text-xs uppercase tracking-widest"
							>
								Exporter
							</button>
						</div>
				...
				>
					...
					<div className="mt-8">
						...
						<h3 className="text-xl font-bold text-taysir-teal/80 uppercase tracking-tighter">
							Séances prévues
						</h3>
						<p className="text-sm text-taysir-teal/50 mt-2 font-medium">
							Répartition sur{" "}
							{todaySessions.length > 0 ? "3 salles" : "0 salle"}.
						</p>
					</div>
				...
				>
					...
					<div>
						<div className="text-3xl font-black text-taysir-teal tracking-tighter">
							{totalPendingAmount.toLocaleString("fr-DZ")}{" "}
							<span className="text-lg opacity-40">DZD</span>
						</div>
				...
				<motion.div className="col-span-1 md:col-span-8 bento-card p-8 flex flex-col md:flex-row items-center gap-8 bg-gradient-to-br from-white to-taysir-teal/5">
					<div className="flex-1">
						...
						<p className="text-sm text-taysir-teal/60 font-medium leading-relaxed max-w-md">
							...
							L&apos;isolation multi-tenant est active pour 1 établissement.
						</p>
						<div className="flex gap-6 mt-6">
							<div className="flex items-center gap-2 text-xs font-bold text-taysir-teal/40 uppercase tracking-widest">
								<CheckCircle2 size={14} className="text-green-500" /> DB
								Connectée
							</div>
							<div className="flex items-center gap-2 text-xs font-bold text-taysir-teal/40 uppercase tracking-widest">
								<CheckCircle2 size={14} className="text-green-500" /> Auth JWT
								OK
							</div>
						</div>
					</div>
					...
					<div className="grid grid-cols-2 gap-3 w-full md:w-auto">
						<div className="bg-white p-4 rounded-2xl border border-taysir-teal/5 shadow-sm text-center">
							<div className="text-xs font-black text-taysir-teal uppercase tracking-tighter opacity-40 mb-1">
								Salles
							</div>
