...
export default function SessionDetails({ session, onSuccess }: SessionDetailsProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [showConfirmDelete, setShowConfirmDelete] = useState(false);
  const [deleteMode, setDeleteMode] = useState<"single" | "following" | "all">("single");
  const [isEditing, setIsEditing] = useState(false);
  const [editMode, setEditMode] = useState<"single" | "following" | "all">("single");

  const handleDelete = () => {
    startTransition(async () => {
      let result;
      if ((deleteMode === "following" || deleteMode === "all") && session.recurrenceGroupId) {
  ...
  const handleUpdate = () => {
    startTransition(async () => {
      let result;
      if ((editMode === "following" || editMode === "all") && session.recurrenceGroupId) {
      ...
      <div className="p-6 rounded-[32px] bg-white border border-taysir-teal/5 shadow-sm">
        ...
        {isEditing ? (
          <div className="p-4 bg-taysir-bg/50 rounded-2xl border border-taysir-teal/5 space-y-4">
            <p className="text-[10px] font-black uppercase text-taysir-teal/40">Mode de modification</p>
            <div className="grid grid-cols-1 gap-2">
               <button
                ...
                >
                  <p className="font-bold uppercase text-[10px]">Cette séance uniquement</p>
                </button>
                {session.recurrenceGroupId && (
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    >
                      <p className="font-bold uppercase text-[10px]">Celle-ci et les suivantes</p>
                    </button>
                    <button
                    ...
                    >
                      <p className="font-bold uppercase text-[10px]">Toute la série</p>
                    </button>
                  </div>
                )}
            </div>
            <p className="text-[10px] italic text-taysir-teal/60">
              Note: Le Drag & Drop sur l&apos;agenda modifie uniquement la séance déplacée.
            </p>
            <SubmitButton
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="flex items-center gap-3 text-taysir-teal">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase opacity-40">Horaire</p>
                <p className="font-bold">
                  {format(new Date(session.startTime), "EEEE d MMMM", { locale: fr })}
                </p>
            ...
            <div className="flex items-center gap-3 text-taysir-teal">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase opacity-40">Localisation</p>
            ...
            <div className="flex items-center gap-3 text-taysir-teal">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase opacity-40">Intervenant</p>
                <p className="font-bold">
        ...
        {!showConfirmDelete ? (
          ...
          <div className="space-y-4 p-6 bg-rose-50 rounded-[32px] border border-rose-100">
            <div className="flex items-center gap-3 text-rose-600 mb-2">
              ...
              <p className="font-black uppercase tracking-tighter">Confirmation</p>
            </div>
            ...
            {session.recurrenceGroupId ? (
              <div className="grid grid-cols-1 gap-2">
                ...
                >
                  <p className="font-bold uppercase text-[10px]">Uniquement cette séance</p>
                </button>
                <button
                ...
                >
                  <p className="font-bold uppercase text-[10px]">Toutes les séances (Série complète)</p>
                </button>
              </div>
            ) : (
