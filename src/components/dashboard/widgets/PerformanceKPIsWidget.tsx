import { TrendingUp, Wallet, ArrowUpRight, Zap, DoorOpen } from 'lucide-react';
import { getFinancialKPIsAction, getDailyAttendanceRatioAction, getAttendanceStatsAction, getRoomOccupancyAction } from '@/actions/dashboard.actions';
import AttendanceSparkline from './AttendanceSparkline';
import Link from 'next/link';

export default async function PerformanceKPIsWidget() {
  const [financeRes, attendanceRes, sparklineRes, occupancyRes] = await Promise.all([
  ...
  const finance = financeRes.success ? financeRes.data : { monthlyRevenue: 0, currency: 'DZD' };
  const attendance = attendanceRes.success ? attendanceRes.data : { ratio: 0 };
  const sparklineData = sparklineRes.success ? sparklineRes.data : [0, 0, 0, 0, 0, 0, 0];
    ...
    <div className="col-span-1 md:col-span-4 bento-card p-8 flex flex-col group hover:border-taysir-accent/20 transition-all duration-500 shadow-sm hover:shadow-xl relative overflow-hidden bg-gradient-to-br from-white to-taysir-bg/30">
      <div className="flex justify-between items-start mb-8 relative z-10">
        <div>
          ...
          <p className="text-[10px] font-bold text-taysir-teal/40 uppercase tracking-widest mt-1">Indicateurs de santé critiques</p>
        </div>
        <Link href="/dashboard/payments" className="p-2 bg-taysir-teal/5 rounded-xl text-taysir-teal hover:bg-taysir-teal hover:text-white transition-all">
      ...
      <div className="grid grid-cols-1 gap-6 h-full relative z-10">
        ...
        <div className="flex flex-col justify-between p-6 bg-white rounded-[24px] border border-taysir-teal/5 shadow-sm group/card hover:border-emerald-500/20 transition-all">
          <div className="flex justify-between items-start">
            ...
            <span className="text-[10px] font-black text-taysir-teal/30 uppercase tracking-widest">Trésorerie Mensuelle</span>
          </div>
          <div className="mt-4">
            <div className="text-4xl font-black text-taysir-teal tracking-tighter leading-none flex items-baseline gap-2">
              ...
              <span className="text-sm opacity-30 font-bold uppercase tracking-widest">{finance.currency}</span>
            </div>
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-[10px] font-black text-emerald-600">Flux Positif</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="p-5 bg-white rounded-[24px] border border-taysir-teal/5 shadow-sm hover:border-taysir-accent/20 transition-all">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[9px] font-black text-taysir-teal/30 uppercase tracking-wider">Engagement</span>
            </div>
            <div className="text-2xl font-black text-taysir-teal">{attendance.ratio}%</div>
            <div className="mt-2 h-1 bg-taysir-teal/5 rounded-full overflow-hidden">
               <div className="h-full bg-taysir-accent" style={{ width: `${attendance.ratio}%` }} />
            </div>
          </div>
          ...
          <div className="p-5 bg-white rounded-[24px] border border-taysir-teal/5 shadow-sm hover:border-taysir-teal/20 transition-all">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[9px] font-black text-taysir-teal/30 uppercase tracking-wider">Occupation</span>
            </div>
            <div className="text-2xl font-black text-taysir-teal">{occupancy.rate}%</div>
            <div className="mt-2 h-1 bg-taysir-teal/5 rounded-full overflow-hidden">
               <div className="h-full bg-taysir-teal" style={{ width: `${occupancy.rate}%` }} />
            </div>
          </div>
        </div>
        ...
        <div className="px-2">
          ...
          <p className="text-[8px] font-bold text-center text-taysir-teal/20 uppercase tracking-[0.3em] mt-2">Tendance hebdomadaire</p>
        </div>
      </div>
