  ...
  return (
    <div className="col-span-1 md:col-span-12 bg-amber-50 border border-amber-200 rounded-[32px] p-6 shadow-sm">
      <div className="flex items-center gap-3 text-amber-700 mb-4">
        ...
        <div>
          <h3 className="text-lg font-black uppercase tracking-tighter">Alertes Opérationnelles</h3>
          <p className="text-xs font-bold opacity-60 uppercase tracking-widest">Sessions imminentes (&lt; 30 min)</p>
        </div>
      </div>

      <div className="flex flex-wrap gap-4">
        {alerts.map((session: any) => (
          <div key={session.id} className="bg-white p-4 rounded-2xl border border-amber-100 shadow-sm flex items-center gap-4 flex-1 min-w-[280px]">
            <div className="w-10 h-10 rounded-full bg-taysir-teal/10 flex items-center justify-center font-bold text-taysir-teal overflow-hidden">
               {session.instructor.avatarUrl ? (
                 <img src={session.instructor.avatarUrl} alt={session.instructor.firstName} className="w-full h-full object-cover" />
               ) : (
                 session.instructor.firstName[0]
               )}
            </div>
            <div className="flex-1">
               <div className="text-sm font-black text-taysir-teal">{session.instructor.firstName} {session.instructor.lastName}</div>
               <div className="text-[10px] font-bold text-taysir-teal/40 uppercase tracking-widest flex items-center gap-1">
                 <Clock size={10} /> {formatTime(session.startTime)} • {session.activity.name} • {session.room.name}
               </div>
            </div>
