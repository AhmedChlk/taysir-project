...
vi.mock("@/lib/prisma", () => {
    const mock = {
        user: { findUnique: vi.fn(), update: vi.fn(), findFirst: vi.fn(), delete: vi.fn() },
...
describe("Settings Actions Audit", () => {
	...
	describe("🔴 A. Sécurité et Permissions (Access Control)", () => {
		it("updateProfileAction: Bloque si l'email est déjà utilisé par un AUTRE compte (Email Conflict)", async () => {
            ...
            vi.mocked(prisma.user.findUnique as any).mockResolvedValue({ id: "other-user", email: "taken@test.com" });
        ...
        it("updateProfileAction: Autorise la mise à jour si l'email appartient au compte courant", async () => {
			vi.mocked(getServerSession).mockResolvedValue(makeSession() as never);
            vi.mocked(prisma.user.findUnique as any).mockResolvedValue({ id: USER_ID, email: "my-email@test.com" });
        ...
        it("updateSchoolAction: Bloque les utilisateurs sans privilèges (ex: STAFF ou STUDENT)", async () => {
			vi.mocked(getServerSession).mockResolvedValue(makeSession({ role: "STAFF" }) as never);
        ...
        it("updateSchoolAction: Autorise GERANT, ADMIN et SUPER_ADMIN", async () => {
            ...
            for (const role of roles) {
                vi.mocked(getServerSession).mockResolvedValue(makeSession({ role }) as never);
                vi.mocked(prisma.etablissement.update).mockResolvedValue({ id: TENANT_ID } as any);
	...
	describe("🟡 C. Résilience et Validation", () => {
		it("updateSchoolAction: Rejette les couleurs mal formatées via Zod", async () => {
            ...
            const res1 = await updateSchoolAction({ name: "School", primaryColor: "red" });
			expect(res1.success).toBe(false);

            const res2 = await updateSchoolAction({ name: "School", primaryColor: "#1234567" }); // Trop long
	...
	describe("🟢 D. Cache et Invalidation", () => {
		it("updateSchoolAction: Déclenche revalidateTag après succès", async () => {
            ...
            expect(revalidateTag).toHaveBeenCalledWith(`etab_${TENANT_ID}_settings`, "max");
		});
        ...
        it("updateProfileAction: Déclenche revalidateTag après succès", async () => {
			...
			await updateProfileAction({ firstName: "John", lastName: "Doe", email: "john@doe.com" });
