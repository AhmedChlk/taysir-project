...
describe("SuperAdmin Actions Audit", () => {
	...
	describe("🔴 Le Mur de Feu (Privilege Escalation)", () => {
		...
		it.each(
			forbiddenRoles,
		)("rejette l'accès si le rôle est %s", async (role) => {
			vi.mocked(getServerSession).mockResolvedValue(
				makeSession(role, "some-etab") as never,
			);
	...
	describe("getAllTenantsAction", () => {
		it("🟢 Accès Global: récupère tous les établissements", async () => {
			vi.mocked(getServerSession).mockResolvedValue(
				makeSession("SUPER_ADMIN") as never,
			);
		...
		it("🟠 Pannes Systèmes: gère une erreur Prisma", async () => {
			...
			vi.mocked(prisma.etablissement.findMany).mockRejectedValue(
				new Error("CRASH"),
			);
	...
	describe("createTenantAction", () => {
		...
		it("Happy Path Extrême: création réussie avec contractEndDate", async () => {
			...
			vi.mocked(prisma.etablissement.create).mockResolvedValue({
				id: "etab-1",
			} as any);
			...
			expect(prisma.etablissement.create).toHaveBeenCalledWith(
		...
		it("rejette si slug déjà pris", async () => {
			...
			vi.mocked(prisma.etablissement.findUnique).mockResolvedValue({
				id: "exist",
			} as any);
			...
			expect(result.success).toBe(false);
			if (!result.success)
				expect(result.error.message).toBe("Ce slug est déjà utilisé.");
		});
			...
			expect(prisma.etablissement.update).toHaveBeenCalledWith(
				expect.objectContaining({
					data: expect.not.objectContaining({
						contractEndDate: expect.anything(),
					}),
				}),
			);
		});
