...
describe("Users Actions Audit", () => {
	...
	describe("createUserAction", () => {
		...
		it("Système: gère une erreur Prisma", async () => {
			vi.mocked(getServerSession).mockResolvedValue(makeSession() as never);
			vi.mocked(prisma.user.findUnique as any).mockRejectedValue(
				new Error("DB_DOWN"),
			);
	...
	describe("updateUserAction", () => {
		it("Escalade de privilèges: restrictions pour les non-gérants", async () => {
			...
			// Rejet rôle
			const res1 = await updateUserAction({
				id: VALID_UUID,
				role: "GERANT" as any,
			});
			...
			// Rejet statut
			const res3 = await updateUserAction({
				id: VALID_UUID,
			...
			const res4 = await updateUserAction({
				id: VALID_UUID,
				firstName: "Nouveau",
			});
		...
		it("Isolation et Erreur: gère le tenant spoofing et ressource absente", async () => {
			vi.mocked(getServerSession).mockResolvedValue(
				makeSession({ etablissementId: "etab-1", role: "GERANT" }) as never,
			);
			...
			expect(mockFindUnique).toHaveBeenCalledWith(
				expect.objectContaining({
					where: {
						id_etablissementId: { id: OTHER_UUID, etablissementId: "etab-1" },
					},
				}),
			);
		});
	...
	describe("resetUserPasswordAction", () => {
		it("Happy Path: réinitialise le mot de passe avec succès", async () => {
			...
			const mockUpdate = vi
				.fn()
				.mockResolvedValue({ id: VALID_UUID, email: "test@test.com" });
			vi.mocked(getTenantPrisma).mockReturnValue({
			...
			expect(mockUpdate).toHaveBeenCalledWith(
				expect.objectContaining({
					where: {
						id_etablissementId: { id: VALID_UUID, etablissementId: "etab-1" },
					},
		...
		it("Zod: rejette un mot de passe trop court pour le reset", async () => {
			vi.mocked(getServerSession).mockResolvedValue(makeSession() as never);
			const resZod = await resetUserPasswordAction({
				id: VALID_UUID,
				newPassword: "123",
			});
		...
		it("Isolation: utilise la clé composite id_etablissementId pour le reset", async () => {
			vi.mocked(getServerSession).mockResolvedValue(
				makeSession({ etablissementId: "target-etab" }) as never,
			);
			const mockUpdate = vi.fn().mockResolvedValue({ id: VALID_UUID });
			vi.mocked(getTenantPrisma).mockReturnValue({
				user: { update: mockUpdate },
	...
	describe("deleteUserAction", () => {
		it("Isolation et Résilience: suppression sécurisée", async () => {
			...
			expect(mockDelete).toHaveBeenCalledWith({
				where: {
					id_etablissementId: { id: VALID_UUID, etablissementId: "my-etab" },
				},
			});
